...
const C = { bgTop:'#0D2818', bgCard:'#FFFFFF', green:'#2E7D3C', lGreen:'#43D854', dark:'#1A1A1A', muted:'#888888', inputBg:'#F5F5F5', border:'#E0E0E0', iconC:'#AAAAAA', white:'#FFFFFF', tagline:'#A0C4A8', link:'#2E7D3C', admin:'#999999' };
    ...
    <KeyboardAvoidingView style={s.root} behavior={Platform.OS==='ios'?'padding':'height'}>
      ...
      <ScrollView contentContainerStyle={s.scroll} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
        ...
        <View style={s.header}>
          ...
          <View style={s.dotsWrap}>{Array.from({length:12}).map((_,i)=><View key={i} style={s.dot}/>)}</View>
        </View>
        ...
        <View style={s.card}>
          ...
          <View style={s.inputWrap}>
            ...
            <TextInput style={s.input} placeholder="••••••••" placeholderTextColor={C.iconC} secureTextEntry={!show} value={pass} onChangeText={setPass}/>
            <TouchableOpacity onPress={()=>setShow(!show)}><Text style={{fontSize:16}}>{show?'🙈':'👁️'}</Text></TouchableOpacity>
          </View>

          <TouchableOpacity style={s.forgotRow} onPress={()=>Alert.alert('Recuperar contraseña','Función disponible próximamente')}>
          ...
          <View style={s.socialRow}>
            <TouchableOpacity style={s.socialBtn} onPress={()=>Alert.alert('Próximamente','Google login disponible pronto')}>
              <Text style={[s.socialIcon,{color:'#DB4437'}]}>G</Text><Text style={s.socialTxt}>Google</Text>
            </TouchableOpacity>
            <TouchableOpacity style={s.socialBtn} onPress={()=>Alert.alert('Próximamente','Facebook login disponible pronto')}>
              <Text style={[s.socialIcon,{color:'#1877F2'}]}>f</Text><Text style={s.socialTxt}>Facebook</Text>
            </TouchableOpacity>
          </View>
